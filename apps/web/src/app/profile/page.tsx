...
export default function ProfilePage() {
  ...
  return <AuthGuard><div className="page-shell">
    <div className="space-y-8">
      ...
      {loading ? <div className="surface space-y-4 p-6"><Skeleton className="h-10 w-full" /><Skeleton className="h-24 w-full" /><Skeleton className="h-24 w-full" /></div> : error && profileId ? <div role="alert" className="status-error flex max-w-xl flex-col justify-between gap-3 text-sm sm:flex-row sm:items-center"><span>{error}</span><button type="button" onClick={() => void loadProfile()} className="btn btn-secondary min-h-[34px] px-3 text-xs">Try again</button></div> : <form onSubmit={saveProfile} className="grid gap-6 lg:grid-cols-[1fr_300px]">
