...
export default function FactsPage() {
  ...
  return <AuthGuard><div className="page-shell space-y-8">
    ...
    {loading ? <div className="surface space-y-4 p-6">{[0, 1, 2].map(i => <Skeleton key={i} className="h-24 w-full" />)}</div> : facts.length === 0 ? <div className="surface-muted p-12 text-center"><ScanSearch aria-hidden className="mx-auto text-accent" size={32} /><h2 className="mt-3 font-semibold text-label">No facts yet</h2><p className="mt-1 text-sm text-muted">Upload a resume to extract evidence for review.</p></div> : <div className="space-y-4">{facts.map(fact => <article key={fact.id} className="surface p-5"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="rounded-md bg-accent-soft px-2.5 py-1 text-[11px] font-bold text-accent-strong">{fact.category}</span><span className="text-xs text-muted">{Math.round(fact.confidence * 100)}% confidence · {fact.sourceRef}</span></div><h2 className="mt-3 text-base font-semibold leading-6 text-ink">{fact.claim}</h2>{fact.context && <p className="mt-2 text-sm leading-6 text-muted">{fact.context}</p>}</div><select aria-label={`Status for ${fact.claim}`} value={fact.status === 'EXTRACTED' ? 'NEEDS_REVIEW' : fact.status} onChange={event => void updateFact(fact.id, event.target.value as Fact['status'])} className="field-control mt-0 w-full sm:w-auto"><option value="NEEDS_REVIEW">Needs review</option><option value="VERIFIED">Verified</option><option value="REJECTED">Rejected</option></select></div></article>)}</div>}
  </div></AuthGuard>;
}
